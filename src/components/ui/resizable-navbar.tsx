...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      // Add -webkit-backdrop-filter for Safari and ensure the container has a translucent
      // background so the blur is visible.
      style={{
        minWidth: "800px",
      ...
      className={cn(
        ...
        // use translucent backgrounds so backdrop-filter has an effect
        visible && "bg-white/10 dark:bg-neutral-950/60",
        className,
      )}
    >
      {children}
...
export const MobileNav = ({ children, className, visible }: MobileNavProps) => {
  return (
    <motion.div
      ...
      // Add -webkit-backdrop-filter for Safari so the blur works there as well.
      style={{ WebkitBackdropFilter: visible ? "blur(10px)" : "none" }}
      className={cn(
